...
#ifndef _HRTF_SOFA_READER_
#define _HRTF_SOFA_READER_
...
{   
	...
	*/
	int inline GetSampleRateFromSofa(const std::string & sofafile)	
	{
		try
		{
	...
	*   \eh On error, an error code is reported to the error handler. */
	...
	bool inline LoadHRTFTableFromSOFA(const std::string& sofafile, shared_ptr<BRTServices::CHRTF> listenerHRTF, bool& specifiedDelays)
	{
		...
		try {
			...
			listenerHRTF->BeginSetup(nSamples, distance);
			...
			for (std::size_t i = 0; i < nMeasurements; i++) // or for( std::size_t i = 0; i < dims[0]; i++ ), should be the same.
			{
				...
				listenerHRTF->AddHRIR(azimuth, elevation, std::move(hrir_value));
			}
	...
	*   \eh On error, an error code is reported to the error handler.
	*/
	bool inline CreateFromSofa(const std::string & sofafile, shared_ptr<BRTServices::CHRTF> listenerHRTF, bool & specifiedDelays)	
	{
